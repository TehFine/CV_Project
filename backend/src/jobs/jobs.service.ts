...
export class JobsService {
  ...
  async findAll(query: any) {
    ...
    const expiryDate = new Date(
      Date.now() - jobsSettings.jobExpiryDays * 24 * 60 * 60 * 1000,
    );
    ...
    const getLenientRegex = (val: string) => {
      if (!val) return null;
      const unaccented = val
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/&/g, 'va');
      const escaped = val.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const escapedUnaccented = unaccented.replace(
        /[.*+?^${}()|[\]\\]/g,
        '\\$&',
      );
  ...
  async findOne(id: string) {
    ...
    try {
      ...
      this.logger.warn('Failed to emit job views update via WebSocket', e);
    }
  ...
  async create(jobData: any, employerId: string) {
    ...
    const activeJobCount = await this.jobModel
      .countDocuments({
    ...
    this.logger.success('Tạo tin tuyển dụng mới', {
      userId: employerId,
      action: 'create_job',
    ...
    if (saved.status === 'pending' || saved.status === 'draft') {
      try {
        ...
        this.logger.log('Đã tạo thông báo cho admin về tin mới', {
          action: 'notify_admin_new_job',
          jobId: saved._id.toString(),
        });
        ...
        this.logger.error('Không thể tạo thông báo admin', notifErr);
      }
    }
  ...
  async update(
    ...
    if (
      requesterRole !== 'admin' &&
      job.employerId?.toString() !== requesterId
    ) {
      this.logger.fail('Cập nhật job thất bại - không có quyền', {
        userId: requesterId,
        action: 'update_job',
        jobId: id,
      });
    ...
    const updated = await this.jobModel
      ...
      .exec();
    this.logger.success('Cập nhật tin tuyển dụng', {
      userId: requesterId,
      action: 'update_job',
      jobId: id,
      title: jobData.title,
  ...
  async remove(id: string, requesterId: string, requesterRole: string) {
    ...
    if (
      ...
      this.logger.fail('Xóa job thất bại - không có quyền', {
        userId: requesterId,
        action: 'delete_job',
        jobId: id,
      });
    ...
    const rejectResult = await this.applicationModel
      .updateMany(
        {
          jobId: id as any,
      ...
      .exec();
    this.logger.log(
      `Đã từ chối ${rejectResult.modifiedCount} đơn ứng tuyển do job bị xóa`,
      { action: 'auto_reject_applications', jobId: id },
    );

    await this.jobModel.findByIdAndDelete(id).exec();
    this.logger.success('Xóa tin tuyển dụng', {
      userId: requesterId,
      action: 'delete_job',
      jobId: id,
      title: job.title,
  ...
  async apply(
    ...
    if (!job) {
      this.logger.fail('Ứng tuyển thất bại - job không tồn tại', {
        userId: candidateId,
        action: 'apply_job',
        jobId,
      });
    ...
    if ((job as any).createdAt < expiryDate) {
      throw new BadRequestException(
        'Tin tuyển dụng này đã hết hạn, không thể ứng tuyển.',
      );
    }
    ...
    if (existingApp) {
      this.logger.fail('Ứng tuyển thất bại - đã ứng tuyển', {
        userId: candidateId,
        action: 'apply_job',
        jobId,
      });
    ...
    if (file) {
      ...
      if (file.size > maxBytes) {
        throw new BadRequestException(
          `Dung lượng file vượt quá giới hạn ${aiSettings.maxFileSizeMB}MB.`,
        );
      }
      const ext = file.originalname.split('.').pop()?.toLowerCase();
      if (!ext || !aiSettings.supportedFormats.includes(ext)) {
        throw new BadRequestException(
          `Định dạng file không được hỗ trợ. Các định dạng: ${aiSettings.supportedFormats.join(', ')}`,
        );
      }
      ...
      if (specificScore) {
        ...
        throw new BadRequestException(
          'Bạn chưa đính kèm CV. Vui lòng đính kèm CV để ứng tuyển.',
        );
      }
    }
    ...
    try {
      ...
      this.logger.warn(
        'Failed to emit job applications update via WebSocket',
        e,
      );
    }
    ...
    this.logger.success('Ứng tuyển thành công', {
      userId: candidateId,
      action: 'apply_job',
      jobId,
      title: job.title,
  ...
  async seedData() {
    ...
    return {
      message: `Da seed ${result.length} jobs vao database`,
      count: result.length,
    };
  }
  ...
  async toggleSaveJob(
    ...
    const idx = (user.savedJobs || []).findIndex(
    ...
    if (idx === -1) {
      ...
      await this.userModel.findByIdAndUpdate(userId, {
        $pull: { savedJobs: jobObjId },
      });
      this.logger.log('Bỏ lưu việc làm', {
        userId,
        jobId,
        action: 'unsave_job',
      });
