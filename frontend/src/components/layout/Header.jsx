...
import { LogOut, User, Bookmark, LayoutDashboard, Briefcase, Users, PlusCircle, Sparkles, FileEdit } from "lucide-react";
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <div style={{ width: 32, height: 32, borderRadius: 8, background: "#1549B8", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ color: "white", fontWeight: 900, fontSize: 14 }}>N</span>
            </div>
            <span style={{ fontWeight: 800, fontSize: 20, letterSpacing: "-0.5px", color: logoColor, transition: "color 0.25s" }}>
            ...
            {navLinks.map((link) => {
              ...
              if (link.highlight) return (
                <Link key={link.href} to={link.href} style={{
                  ...
                  backgroundColor: "#1549B8", color: "white",
              ...
              return (
                <Link key={link.href} to={link.href} style={{
                  ...
                  {link.label}
                  {link.badge && (
                    <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 6px", borderRadius: 99, background: "#EDE9FE", color: "#6D28D9" }}>
                      {link.badge}
                    </span>
                  )}
                </Link>
          ...
          <div className="hidden md:flex" style={{ alignItems: "center", gap: 8, marginLeft: "auto" }}>
            ...
            {!isEmployer && (
              <button
                ...
                onMouseEnter={(e) => e.currentTarget.style.backgroundColor = scrolled || !isHome ? "#EEF2FF" : "rgba(255,255,255,0.15)"}
            ...
            {isAuthenticated ? (
              ...
              <div id="user-dropdown" style={{ position: "relative" }}>
                <button
                  ...
                  onMouseEnter={(e) => e.currentTarget.style.backgroundColor = scrolled || !isHome ? "#F1F5F9" : "rgba(255,255,255,0.15)"}
                  onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
                >
                  {/* Avatar */}
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#1549B8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "white", flexShrink: 0 }}>
                    {getInitials(user?.name)}
                  </div>
                  <div style={{ textAlign: "left" }}>
                    <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.2, color: scrolled || !isHome ? "#0F172A" : "white" }}>
                      {user?.name?.split(" ").slice(-1)[0]}
                    </div>
                    <div style={{ fontSize: 11, color: scrolled || !isHome ? "#94A3B8" : "rgba(255,255,255,0.7)" }}>
                      {isEmployer ? (user?.companyName || "Nhà tuyển dụng") : "Ứng viên"}
                    </div>
                  </div>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={scrolled || !isHome ? "#94A3B8" : "rgba(255,255,255,0.7)"} strokeWidth="2">
                    <path d="M6 9l6 6 6-6" />
                  </svg>
                </button>
                ...
                {dropdownOpen && (
                  <div style={{
                    ...
                    width: 228, backgroundColor: "white", borderRadius: 12,
                    boxShadow: "0 8px 30px rgba(0,0,0,0.12)", border: "1px solid #E2E8F0",
                    overflow: "hidden", zIndex: 100, animation: "fadeInUp 0.15s ease",
                  }}>
                    ...
                    ].map((item) => <DDLink key={item.href} {...item} onClick={() => setDropdownOpen(false)} />)}

                    {isEmployer && [
                      { href: "/employer/dashboard",  icon: <LayoutDashboard size={14} />, label: "Dashboard" },
                      { href: "/employer/post-job",   icon: <PlusCircle size={14} />,      label: "Đăng tin tuyển dụng" },
                      { href: "/employer/candidates", icon: <Users size={14} />,           label: "Quản lý ứng viên" },
                    ].map((item) => <DDLink key={item.href} {...item} accent onClick={() => setDropdownOpen(false)} />)}

                    <div style={{ borderTop: "1px solid #E2E8F0" }}>
                      <button
                        onClick={handleLogout}
                        style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 16px", fontSize: 13, color: "#DC2626", background: "none", border: "none", cursor: "pointer", textAlign: "left", fontFamily: "inherit", transition: "background 0.15s" }}
              ...
              <>
                ...
                <Link to="/register" style={{
                  ...
                  backgroundColor: "#1549B8", color: "white", textDecoration: "none",
                  boxShadow: "0 2px 8px rgba(21,73,184,0.3)", transition: "all 0.2s",
                }}>Đăng ký</Link>
                <Link to="/register?role=employer" style={{
                  padding: "7px 16px", borderRadius: 8, fontSize: 14, fontWeight: 600,
                  backgroundColor: "#0F172A", color: "white", textDecoration: "none", transition: "all 0.2s",
                }}>Nhà tuyển dụng</Link>
              </>
            )}
          </div>
            ...
            <Sheet>
              <SheetTrigger asChild>
                <button style={{ background: "transparent", border: "none", cursor: "pointer", padding: 4 }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none"
                    stroke={scrolled || !isHome ? "#475569" : "white"} strokeWidth="2">
              ...
              <SheetContent side="right" className="w-72 p-0 flex flex-col">
                <div className="p-5 border-b">
                  ...
                  {isAuthenticated && (
                    <div className="flex items-center gap-2.5 p-2.5 bg-[#F1F5F9] rounded-lg">
                      <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#1549B8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "white" }}>
                        {getInitials(user?.name)}
                      </div>
                      <div>
                ...
                <div className="p-4 border-t space-y-2">
                  {isAuthenticated ? (
                    <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium bg-red-50 text-red-600 hover:bg-red-100 transition-colors">
                      <LogOut className="h-4 w-4" /> Đăng xuất
                    </button>
                  ) : (
                    <>
                      <SheetClose asChild>
                        <Link to="/login" className="block w-full py-2.5 text-center rounded-lg text-sm font-medium border border-[#E2E8F0] text-[#0F172A] hover:bg-[#F1F5F9] transition-colors" style={{ textDecoration: "none" }}>
                          Đăng nhập
                        </Link>
                      </SheetClose>
                      <SheetClose asChild>
                        <Link to="/register" className="block w-full py-2.5 text-center rounded-lg text-sm font-semibold bg-[#1549B8] text-white hover:bg-[#1240A0] transition-colors" style={{ textDecoration: "none" }}>
                          Đăng ký ứng viên
                        </Link>
                      </SheetClose>
                      <SheetClose asChild>
                        <Link to="/register?role=employer" className="block w-full py-2.5 text-center rounded-lg text-sm font-semibold bg-[#0F172A] text-white hover:bg-slate-800 transition-colors" style={{ textDecoration: "none" }}>
                          Nhà tuyển dụng
                        </Link>
                      </SheetClose>
                    </>
...
function DDLink({ href, icon, label, onClick, accent = false }) {
  return (
    <Link
      ...
      onMouseEnter={(e) => e.currentTarget.style.backgroundColor = accent ? "rgba(124,58,237,0.06)" : "#F8FAFC"}
      onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
    >
      {icon} {label}
    </Link>
